...
namespace System.Processes
{
	...
	/// </summary>
	public class ProcessRunner : IRunner, IDisposable
    {
        ...
        private bool _usesAdditionalParametersFirst;
        private bool _isElevated;
        private NetworkCredential _credential;

		/// <summary>Creates a ProcessRunner for the given executable </summary>
		public ProcessRunner(string executable)
			: this(executable, null, false, false, EmptyArgList)
		{ }
		/// <summary>Creates a ProcessRunner for the given executable and arguments </summary>
        public ProcessRunner(string executable, NetworkCredential credential, bool isElevated, bool usesAdditionalParametersFirst, params string[] args)
		{
			...
			_running = null;
			_stdIn = null;
            _usesAdditionalParametersFirst = usesAdditionalParametersFirst;
		...
		public int Run(TextReader input, params string[] arguments)
		{
            ...
            if (_usesAdditionalParametersFirst)
            {
                args = new List<string>(arguments);
                args.AddRange(_arguments ?? EmptyArgList);
            }
            else
            {
		...
		public void Start(params string[] moreArguments)
		{
            ...
            if (_usesAdditionalParametersFirst)
            {
                args = new List<string>(moreArguments);
                args.AddRange(_arguments ?? EmptyArgList);
            }
            else
            {
		...
		private void InternalStart(params string[] arguments)
		{
            ...
            else
            {
                ...
                if (_credential != null)
                {
                    psi.Domain = _credential.Domain;
                    psi.UserName = _credential.UserName;
                    psi.Password = _credential.Password.ConvertToSecureString();
                }
            }
            ...
            if (!_isElevated)
            {
                if (_running.StartInfo.RedirectStandardInput)
                    _stdIn = _running.StandardInput;

                if (_running.StartInfo.RedirectStandardOutput)
                    _running.BeginOutputReadLine();

                if(_running.StartInfo.RedirectStandardError)
